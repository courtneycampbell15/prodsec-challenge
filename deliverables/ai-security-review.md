...
- **Finding summary**: 2 - 3 sentences that detail what the PR covers and what it may introduce as a risk and what it could impact. It must be very focused on keeping this short and sweet. 
...
- **Block/comment/escalate decision**: 
  ...
  - **Block**: if the PR contains any hardcoded secrets or if Semgrep reports ERROR (including custom BAC/IDOR rules and additional custom rules) with deterministic evidence from the assistant, the PR should be blocked.
  - **Comment**: probably the most used decision, the reviewer should always leave comments on each PR 
  - **Escalate**: may take some focused testing time from the ProdSec team, but if the assistant can’t necessarily determine if the findings it identified are reproducible if it doesn’t have enough context for an authorization workflow, it should escalate to ProdSec and attach its notes.
...
- **Avoid sending secrets to third-party models**: Implement tooling (open source possibly) to redact secrets from prompts, avoid reviewing the .env file if it's accidentally committed, avoid secrets accidentally added historically in the repo, and reviewer should use models approved internally (private or self hosted as an option). 
...
> Metrics should include context around the usefulness of the assistant’s reviews and how quickly it can do its reviews. We could also test models against each other during the early stages of testing to see which may perform better depending on whether we prioritize accuracy, speed, or both. 

- **Seeded test cases**: There should be multiple fake PRs that introduce vulnerabilities, like IDORs, avoiding authentication or authorization checks in code, or possible SSRF vulnerabilities. The goal here is to make sure vulnerable code is caught by the assistant and is ranked with the correct severity. This could be specific to the repo or we could develop a guide that covers a lot of different vulnerable patterns, similar to the OWASP Juice Shop app structure.  
- **False positive rate**: The ProdSec team measures when they review the assistant’s findings and determine they’re either not actionable or inaccurate. This could also be measured by tracking how often engineers dismiss or ignore the review and suggestions as well. 
